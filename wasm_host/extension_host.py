...
async def _dispatch_tag_watch(payment: Payment, watch: TagWatch) -> None:
    try:
        ...
        _check_quota(
            ...
            watch.ext_id,
            "db",
            get_cached_wasm_settings().max_db_ops_per_min,
        )
...
async def _apply_list_updates(
    db: Database, ext_id: str, updates: list[dict[str, Any]]
) -> None:
    if not updates:
        return
    for update in updates:
        ...
        if item_id is None:
            continue
        raw = await _kv_get(db, ext_id, key)
        if not raw:
            continue
        try:
        ...
        if changed:
            ...
            await _kv_set(db, ext_id, key, updated)
    ...
    async def _watch():
        try:
            ...
            if existing and existing.pending is False:
                ...
                payload_json = existing.json(exclude={"preimage"})
            ...
            while True:
                ...
                if (
                    ...
                    and payment.checking_id != payment_hash
                ):
                    continue
                ...
                if payment.pending is False:
                    payload_json = payment.json(exclude={"preimage"})
                    await _kv_set(db, ext_id, store_key, payload_json)
                    await websocket_updater(f"{ext_id}:{store_key}", payload_json)
                    watch_payload = {
                        "payment_hash": payment_hash,
